 ...
 * 
 * Fetches unread emails from Gmail and Outlook, stores them in Vercel Postgres via Drizzle.
      ...
      if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_REFRESH_TOKEN) {
        ...
        for (const email of gmailMessages) {
          try {
            ...
            errors.push(`Gmail ${email.email_id}: ${error.message}`);
          }
        }
      } else {
      ...
      if (process.env.MICROSOFT_CLIENT_ID && process.env.MICROSOFT_TENANT_ID) {
        ...
        for (const email of outlookMessages) {
          try {
            ...
            errors.push(`Outlook ${email.email_id}: ${error.message}`);
          }
        }
      } else {
